  ...
  <section class="hero">
    <div class="container my-5">
      <div class="card card-body shardow text-center">
        ...
        <p class="fst-italic fw-bold text-decoration-line-through">This form should be completed and submitted on or before 28 February, 2026</p>
        <p class="lead">Application is closed for 2026 intake</p>
      </div>
    </div>
  </section>
  <section>
    <div class="container my-5">
      ...
      <?php flash('msg'); ?>
      <?php $isAdmin = isset($_SESSION['admin']); ?>
      <?php if (!$isAdmin): ?>
        <div class="alert alert-warning">Only administrators are allowed to submit applications.</div>
      <?php endif; ?>
      <form method="POST" action="<?= URLROOT; ?>/application/passport" enctype="multipart/form-data" data-parsley-validate <?php if (!$isAdmin) echo 'onsubmit="return false;"'; ?>>
        <input type="hidden" name="reg_id" value="<?= (empty($data['reg_id'])) ? '' : $data['reg_id']; ?>">
        <fieldset <?php if (!$isAdmin) echo 'disabled'; ?>>
          <div class="row">
            <div class="col-md-6">
              <div class="mb-3">
      ...
      <form method="POST" action="<?= URLROOT; ?>/application/step3" data-parsley-validate <?php if (!$isAdmin) echo 'onsubmit="return false;"'; ?>>
        <input type="hidden" name="reg_id" value="<?= (empty($data['reg_id'])) ? '' : $data['reg_id']; ?>">
        <fieldset <?php if (!$isAdmin) echo 'disabled'; ?>>
          <div class="mb-3">
