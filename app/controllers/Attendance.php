  ...
  public function record($set)
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      if ($_POST['morning'] == 'morning') {
        ...
        $marked = $this->attendanceModel->check_attendance1($data['std_id'], $data['day'], $data['mitre_set'], $data['conclave']);
        if ($marked) {
          echo "
                  <div class='flash-msg alert alert-danger'>
                    Attendance already marked for " . $data['name'] . " </span>
                </div>
        ...
        $marked = $this->attendanceModel->check_attendance2($data['std_id'], $data['day'], $data['mitre_set'], $data['conclave']);
        if ($marked) {
          echo "
                  <div class='flash-msg alert alert-danger'>
                    Attendance already marked for " . $data['name'] . " </span>
